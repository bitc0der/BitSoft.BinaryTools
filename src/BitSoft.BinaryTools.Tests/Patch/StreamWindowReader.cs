...
public class StreamWindowReaderTests
{
    ...
    public async Task Should_ReturnInitialWindow()
    {
        ...
        Assert.That(reader.Window.ToArray(), Is.EqualTo(source.AsMemory(0, 2).ToArray()).AsCollection);
    }
    ...
    public async Task Should_ReturnMovedWindow()
    {
        ...
        // Assert
        Assert.That(reader.Window.Length, Is.EqualTo(2));
        Assert.That(reader.Window.ToArray(), Is.EqualTo(source.AsMemory(1, 2).ToArray()).AsCollection);
    }

    [Test]
    public async Task Should_ReturnMovedWindow_When_Overlappeds()
    {
        // Arrange
        var source = new byte[] { 0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6 };
        using var sourceStream = new MemoryStream(source);
        ...
        // Assert
        Assert.That(reader.Window.Length, Is.EqualTo(2));
        Assert.That(reader.Window.ToArray(), Is.EqualTo(source.AsMemory(3, 2).ToArray()).AsCollection);
    }
}
