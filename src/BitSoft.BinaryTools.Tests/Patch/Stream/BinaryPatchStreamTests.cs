...
public class BinaryPatchStreamTests
{
    ...
    {
        yield return new TestCaseData(new byte[] { 0x0, 0x1, 0x2 }, new byte[] { 0x0, 0x1, 0x3 });
        yield return new TestCaseData(new byte[] { 0x0, 0x1, 0x2 }, new byte[] { 0x0, 0x3 });
        yield return new TestCaseData(new byte[] { 0x0, 0x1 }, new byte[] { 0x0, 0x1, 0x3 });
    }
    
    [TestCaseSource(nameof(GetTestCaseData))]
    public async Task Should_WriteToStream(byte[] original, byte[] modified)
    {
        // Arrange
